...
CubemapTexture::CubemapTexture(
                               ...
                               const String& NegZFilename):Texture()
{
...
void TextureManager::Clear()
{
    ...
    auto first = m_textures.first();
    while(first)
    {
        first->Drop();
        first = m_textures.next();
    }
    m_textures.clear();
}
...
void TextureManager::Init()
{
        ...
        m_defaultTexture->LoadFromMemory(data, 4, 1, 1);
...
Texture2D *TextureManager::Get(const Pixmap &pixmap, const char *name)
{
    ...
    if (m_textures.find(name, texture))
    {
        return texture;
    } else 
    { 
...
{
     Texture2D *texture =0x0;
    if (m_textures.find(file_name, texture))
    {
        m_textures.erase(file_name);
        texture->Drop();
        return true;
    }
    return false;
...
Texture2D *TextureManager::Get(const char *name)
{
    ...
    Texture2D *texture=0x0;
    if (m_textures.find(path.c_str(), texture))
    {
    ...
    }else
    {
        ...
        {
             m_textures.insert (path.c_str(),texture);
        } else 
        {
            texture->Drop();
            texture = m_defaultTexture;
...
{
    String path =m_texturePath+ name;
