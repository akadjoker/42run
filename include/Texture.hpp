...
class CORE_PUBLIC Texture : public Ref
{
    public:
        Texture();
        virtual ~Texture();
...
class  CORE_PUBLIC Texture2D  : public Texture
{
    public:
    Texture2D();
...
class CORE_PUBLIC CubemapTexture : public Texture
{
public:
...
class TextureManager 
{
    ...
    Texture2D *Create(const unsigned char *buffer,u16 components, int width, int height);
    Texture2D *Create(const Pixmap &pixmap);
    Texture2D *Create(const char* file_name);
    Texture2D *Create(u16 components, int width, int height);

    String GetTexturePath() const {return m_texturePath;}
